...
>
  ...
  <div className="text-center mb-10">
    ...
    <p className="text-sm md:text-base text-gray-600 max-w-4xl mx-auto leading-relaxed">
      Easy to install | Good Quality Material | Delivery on time | Decent look | Color customization
      <br />
  ...
  <div className="relative">
    ...
    <button
      onClick={prevSlide}
      className="hidden md:flex absolute left-0 top-1/2 -translate-y-1/2 -translate-x-4 md:-translate-x-6 z-10 bg-orange-500 hover:bg-orange-600 text-white p-3 rounded-full shadow-lg transition-all duration-300 transform hover:scale-110"
    >
      <ChevronLeft size={24} />
    </button>
    ...
    <button
      onClick={nextSlide}
      className="hidden md:flex absolute right-0 top-1/2 -translate-y-1/2 translate-x-4 md:translate-x-6 z-10 bg-orange-500 hover:bg-orange-600 text-white p-3 rounded-full shadow-lg transition-all duration-300 transform hover:scale-110"
    >
      <ChevronRight size={24} />
    </button>

    {/* Products */}
    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6 px-6 sm:px-8 md:px-12">
      {getVisibleProducts().map((product, index) => (
        ...
        >
          <div className="aspect-[4/3] sm:aspect-[3/2] md:aspect-[4/3] overflow-hidden">
            <img
              src={product.image}
              alt={product.title}
          ...
          <div className="p-4 bg-white border-t-4 border-orange-500">
            <h3 className="text-base sm:text-lg md:text-base font-bold text-gray-900 group-hover:text-orange-600 transition-colors duration-300">
              - {product.title}
            </h3>
          </div>
        </div>
